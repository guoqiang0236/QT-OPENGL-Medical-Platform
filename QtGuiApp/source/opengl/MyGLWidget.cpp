...
namespace MyOpenGL {
  ...
  //       m_scene->addChild(boxmesh);
    ...
    void MyGLWidget::prepareAssimpInstance()
    {
        ...
        grassMaterial->mCloudMask = new MyTexture("../assets/textures/CLOUD.png", 2);
		...
		auto house = MyAssimpLoader::load("../assets/fbx/house.fbx");
		house->setScale(glm::vec3(0.5f));
        house->setPosition(glm::vec3((rNum - 1) * 0.2f / 2.0f, 0.4f, (cNum - 1) * 0.2f / 2.0f));
		m_scene->addChild(house);
